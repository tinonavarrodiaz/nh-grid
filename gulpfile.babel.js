...
gulp.task('images-dev', () => {
  gulp.src('./src/img/**/**').pipe(gulp.dest('./docs/assets/img'));
});

// Copia html de testing
gulp.task('html', () => {
  gulp.src('./test.html').pipe(gulp.dest('./docs/'));
});
...
gulp.task(
  'dev',
  ['styles-dev', 'pug-dev', 'scripts-dev', 'images-dev', 'html'],
  () => {
    server.init({
      server: {
        baseDir: './docs',
    ...
    gulp.watch('./src/js/**/**', () =>
      gulp.start('scripts-dev', server.reload)
    );
